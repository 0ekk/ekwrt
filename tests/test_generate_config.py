...
class GenerateConfigTests(unittest.TestCase):
    def test_writes_expected_x86_64_config(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("# CONFIG_BUILDBOT is not set", content)
            self.assertIn("# CONFIG_PER_FEED_REPO is not set", content)
            self.assertIn("CONFIG_SIGNED_PACKAGES=y", content)
            self.assertIn("CONFIG_SIGN_EACH_PACKAGE=y", content)
            self.assertIn("CONFIG_PACKAGE_ekwrt-keyring=m", content)
